...
notes[0] = new Map();
notes[0].set("sharp", "B#");
notes[0].set("natural", "C");
notes[0].set("flat", "Dbb");
notes[1] = new Map();
notes[1].set("sharp", "C#");
notes[1].set("natural", "");
notes[1].set("flat", "Db");
notes[2] = new Map();
notes[2].set("sharp", "C##");
notes[2].set("natural", "D");
notes[2].set("flat", "Ebb");
notes[3] = new Map();
notes[3].set("sharp", "D#");
notes[3].set("natural", "");
notes[3].set("flat", "Eb");
notes[4] = new Map();
notes[4].set("sharp", "D##");
notes[4].set("natural", "E");
notes[4].set("flat", "Fb");
notes[5] = new Map();
notes[5].set("sharp", "E#");
notes[5].set("natural", "F");
notes[5].set("flat", "Gbb");
notes[6] = new Map();
notes[6].set("sharp", "F#");
notes[6].set("natural", "");
notes[6].set("flat", "Gb");
notes[7] = new Map();
notes[7].set("sharp", "F##");
notes[7].set("natural", "G");
notes[7].set("flat", "Abb");
notes[8] = new Map();
notes[8].set("sharp", "G#");
notes[8].set("natural", "");
notes[8].set("flat", "Ab");
notes[9] = new Map();
notes[9].set("sharp", "G##");
notes[9].set("natural", "A");
notes[9].set("flat", "Bbb");
notes[10] = new Map();
notes[10].set("sharp", "A#");
notes[10].set("natural", "");
notes[10].set("flat", "Bb");
notes[11] = new Map();
notes[11].set("sharp", "A##");
notes[11].set("natural", "B");
notes[11].set("flat", "Cb");
...
function set_notes() {
  ...
  // determine quality of key signature
  ...
  for (const scale_degree of formula.split(" ")) {
    ...
    // determine note names (e.g. Bb vs A#)
    var note = '';
    var notelist = notes[key_offset]; 
    if (i == 1) {
      note = keyname;
    } else if (notelist.get('natural') != '') {
      natural = true;
      note = notelist.get('natural');
    } else if (sharpkey) {
      sharpnote = true;
      note = notelist.get('sharp');
    } else if (flatkey) {
      flatnote = true;
      note = notelist.get('flat');
    }
    const remove_sharp_regex = /\#*\#$/;
    const remove_flat_regex = /b*b$/;
    if (note.match(remove_sharp_regex)) {
      var base_note = note.replace(remove_sharp_regex, '');
    } else {
      var base_note = note.replace(remove_flat_regex, '');
    }
    console.log(note);

    if (notenames.includes(base_note)) {
      if ((natural) && (sharpkey)) {
        note = notelist.get('sharp');
        var base_note = note.replace(remove_sharp_regex, '');
      } else {
    ...
    for (let [x, y] of tones) {
      if (i == 1) {
        //red
        stroke_color = "rgba(255, 40, 80, 1)";
      } else if ( i == 2 ){
        //orange
        stroke_color = "rgba(220, 100, 0, 1)";
      } else if ( i == 3 ){
        //yellow
        stroke_color = "rgba(230, 230, 40, 1)";
      } else if ( i == 4 ){
        //green
        stroke_color = "rgba(75, 220, 50, 1)";
      } else if ( i == 5 ){
        //blue
        stroke_color = "rgba(0, 220, 220, 1)";
      } else if ( i == 6 ){
        //indigo
        stroke_color = "rgba(150, 75, 255, 1)";
      } else if ( i == 7 ){
        //violet
        stroke_color = "rgba(255, 0, 175, 1)";
      } else {
        //black
        stroke_color = "rgba(0, 0, 0, .9)";
      } 
      ...
      text.setAttributeNS(null, 'style', 'font-size: 1em; font-weight: normal; fill: white; stroke: white; stroke-width: 1px; text-shadow: 0 0 6px rgba(0, 0, 0, .6);' );
      text.textContent = note;
      notes_g.classList.add("notes");
      notes_g.setAttributeNS(null, 'style', 'display: none;')
      notes_g.append(circle);
      notes_g.append(text);
      svg.append(notes_g);
      id++;
    }
